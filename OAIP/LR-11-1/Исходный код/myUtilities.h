#ifndef LR_7_MYUTILITIES_H
#define LR_7_MYUTILITIES_H
...
inline int getIntFromInput() {
    ...
    while (!regex_match(trim_str(str), regex(R"(^(\+|-)?\d+$)"))) {
        cout << "Неверный формат целочисленного значения! Введите снова!" << endl;
        getline(cin, str);
    }
...
inline float getFloatFromInput() {
    ...
    while (!regex_match(trim_str(str), regex(R"(^[-+]?\d*\.?\d*$)"))) {
        cout << "Неверный формат вещественного значения! Введите снова!" << endl;
        getline(cin, str);
    }
...
inline float getFloat(const string &title, double minValue, double maxValue) {
    float value;
...
inline double getDouble(const string &title, double minValue, double maxValue) {
    double value;
...
inline int getInt(const string &title, const vector<int> &codes, const string &outOfRangeMsg) {
    ...
    while (find(codes.begin(), codes.end(), value) == codes.end()) {
        ...
        cout << endl << "Значение должно быть в пределах допустимого набора:" << endl;
